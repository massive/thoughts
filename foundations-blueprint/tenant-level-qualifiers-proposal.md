...
Plan needs isolation between UAT-for-testing and UAT-for-development. Store needs sharing between country-specific deployments within the same tenant. These appear contradictory until you realize they're operating at different levels.
...
### 1. Tenant is the integration boundary

All deployments within a tenant share integration resources. Data flows freely between `circlek-eu-prod-plan` and `circlek-eu-prod-store-pl` because they're in the same tenant.

### 2. Qualified tenants are fully isolated

`circlek-eu-uat` and `circlek-eu-uat-dev` are separate tenants. They have different UUIDs, different Kafka topics, different blob containers. No automatic data sharing.

### 3. Deployment qualifiers don't split tenants

`circlek-eu-prod-store-pl` and `circlek-eu-prod-store-es` are different deployments in the SAME tenant. They share integration resources. This is the Store pattern.

### 4. Tenant qualifiers DO split tenants

`circlek-eu-uat` and `circlek-eu-uat-dev` are DIFFERENT tenants. Complete isolation. This is the Plan pattern when multiple isolated environments are needed within the same environment type.
...
- `circlek-eu-uat-store-pl` - base tenant, Store deployment with country deployment qualifier
...
Each purchased product gets deployments in both tenants automatically.

### Qualified tenants (manual via CMT)

When additional isolation is needed:
...
This keeps infrastructure costs controlled - qualified tenants are created when explicitly needed, not by default.
